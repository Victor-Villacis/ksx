...
mod tests {
    ...
    fn clean_future_machine_is_quiet() {
        ...
        // The gated retro pair keeps the gate note alive.
        assert_eq!(
            codes(&advice),
            vec![
    ...
    fn installing_hidmaestro_changes_machine_readiness_not_build_capability() {
        ...
        // The gated retro pair keeps the note alive in BOTH states — the
        // property this test pins: an install must not change what the build
        // can plug.
        assert_eq!(
        ...
        for (label, report) in [("absent", &absent), ("installed", &present)] {
            ...
            assert!(gate.message.contains("snes"), "{label}: {}", gate.message);
            assert!(
                gate.message.contains("genesis"),
                "{label}: {}",
                gate.message
            );
