...
//! still has one backend verb per step. The shared staged-workbench operations
//! live in `server/workbench.rs`; `/redesign` supplies the current form doors
//! and 303 homes. The learner behind the proving step is read back per render
//! so it works with scripting switched off. A verb that moves here and loses
//! its no-JS path has silently stopped being reachable for the audience the
//! page was built for.
//!
