...
impl Client {
    ...
    pub fn connect() -> Result<Client, Error> {
        unsafe {
            ...
            ) != 0
            {
                ...
                let detail_data_ptr =
                    detail_data_buffer.as_mut_ptr() as PSP_DEVICE_INTERFACE_DETAIL_DATA_W;
                *ptr::addr_of_mut!((*detail_data_ptr).cbSize) =
    ...
    pub fn try_clone(&self) -> Result<Client, Error> {
        unsafe {
            ...
            Ok(Client {
                device: target_handle.assume_init(),
            })
        }
    }
}
