...
| M8 | 🔨 catalog/host/rendezvous contracts and structural package gate done; production transport and adapter absent | HIDMaestro rich-profile Windows backend: DualSense, Switch Pro, Xbox Series and catalog expansion | The incompatible latch adapter was removed. A non-executing pinned SDK catalog probe, bounded host contract, structural unsigned-candidate audit and pure one-use rendezvous policy now exist. The policy fixes token/name/argv and requires authenticated exact-process evidence, but creates no pipe and launches nothing; authenticated OS transport, a sanitized runtime-only SDK, signed packages and disposable-machine lifecycle evidence remain. Personas stay gated until that complete path passes — see `docs/HIDMAESTRO.md` |
