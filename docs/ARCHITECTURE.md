...
| M8 | 🔨 S1.5c source contracts and SDK-free transport proven; runtime/driver artifacts and adapter absent | HIDMaestro rich-profile Windows backend: DualSense, Switch Pro, Xbox Series and catalog expansion | [Actions run 31860063900](https://github.com/Victor-Villacis/ksx/actions/runs/31860063900) freezes the one-USB-DualSense source target: 9 public types/100 entries, exhaustive 51-unit source disposition, the 231-file → 228-resource/130-deployable source catalog, and one shared 16-vector raw-feedback reducer. It does not provide managed replacements, a built runtime, catalog binding, managed decoder, signed-driver ABI, distribution approval or hardware lifecycle evidence. Next is the inert replacement-source tree; personas remain gated — see `docs/HIDMAESTRO.md` |
