...
| M8 | 🔨 catalog/host contracts, structural package gate and SDK-free authenticated transport proven; production host and adapter absent | HIDMaestro rich-profile Windows backend: DualSense, Switch Pro, Xbox Series and catalog expansion | The incompatible latch adapter was removed. A non-executing pinned SDK catalog probe, bounded host contract, structural unsigned-candidate audit and one-use rendezvous now exist. [Actions run 31852136380](https://github.com/Victor-Villacis/ksx/actions/runs/31852136380) additionally proved the exact runtime-source baseline and pure native-bootstrap policy, then passed the SDK-free authenticated pipe, workspace and release/installer gates. No runtime-only SDK or production bootstrap/managed host was built, elevated or made reachable. Signed packages and disposable-machine lifecycle evidence remain. Personas stay gated until that complete path passes — see `docs/HIDMAESTRO.md` |
