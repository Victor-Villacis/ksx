...
| **Inspect panel encoder hardware** | `ksx panel status [--device QUERY] [--json]` — groups one physical encoder, reports raw USB identity and passive HID collection capabilities, and says plainly that exact mode/chart protocol is unverified. It sends no feature/output report, changes no binding or EEPROM state, and deliberately has no `--yes` | **Studio's Control Surface Builder fetches the same `MachineSource::panel_status` view on open or Refresh**, outside the 2 s poll. The card is context for Design / Teach / Route, never a fourth programming stage | exists — read-only CLI + Studio view; chart read/write remain E10 work |
