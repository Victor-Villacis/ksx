...
|---|---|---|---|
| Start emulation | `ksx run` (foreground session); daemon: tray "Start emulation" / headless stdin `start` / pipe `start` / `ksx session start [--game TITLE]` — all → `DaemonCommand::Start` | M9: enqueue `DaemonCommand::Start` on the control loop the UI hosts in-process. **M10: Studio's Play button POSTs `/redesign/play` → pipe `start` → the same command (live)** | exists — pipe + CLI + Studio live |
| Stop emulation | tray "Stop emulation" / stdin `stop` / pipe `stop` / `ksx session stop` → `DaemonCommand::Stop`; Ctrl+Alt+Del is the capture-thread emergency stop. LeftCtrl ×5 only toggles keyboard capture off/on and deliberately leaves the session running | M9: `DaemonCommand::Stop`. **M10: Studio's Stop button POSTs `/redesign/stop` → pipe `stop` (live).** Escapes are deliberately NOT a GUI concern — see invariants | exists — pipe + CLI + Studio live |
| **Resume emulation** (put back what `stop` stopped) | pipe `resume` / `ksx session resume`. Takes NO argument: the daemon answers from what it started (`DaemonState::origin`), so a staged session resumes staged — re-committed from the setup as it stands, edits and all — and a profile session resumes that profile. A refusal names what is missing and says nothing staged was discarded | M9: the same verb in-process. No tray item: the tray never paused anything, and its Start is the config on disk by design. **Studio's face is GONE as of 2026-08-25**: the mapper used to POST `/api/session/resume` after a "Pause emulation & map" button, and the cutover deleted the route, the button and the flow it belonged to. `ControlSource::resume` now has zero Studio callers — a backend verb whose second caller went away, which `SURFACES.md` §1 says is not a finished state | exists — pipe + CLI; **no Studio face** |
...
| **Measure simultaneous keyboard / encoder host signals** | Typed pipe `input-test-start` / `input-test-poll` / generation-stamped `input-test-cancel`; server-internal `input-test-release-fence` gives a terminal observer one bounded teardown handoff before another hardware transaction; CLI: `ksx input-test start SELECTOR [--duration-ms 1000..=60000]`, then `poll`, and `cancel --generation N` | The bounded `/api/input-test*` adapter remains available, but its Keyboard Workbench / Control Surface Builder UI is deferred under `DEFERRED-SURFACES.md`. Exact selected selector, bounded wall-clock observation, no mapping or hardware mutation. Reports host-visible `held`, `seen`, `peak`, `events`, `dropped` and explicitly unavailable rollover visibility; duplicate terminals emitting one key cannot be distinguished | exists — pipe + CLI + API; no Studio UI |
...
| Game profiles | TOML edit (`games.toml`); consumed by `ksx run --game`, `ksx daemon --game`, `ksx autostart --game` | Editing retains typed backend planners, with future GUI forms over them. Consuming: `DaemonCommand`/api as above. **The cabinet's "Game" screen picks one and starts it** — one `start` with a profile title, never a new path. Studio game authoring and saved-profile switching have no UI and remain explicitly deferred to Settings/Library | gap for Studio authoring; consuming and cabinet picking exist |
| **Which preset a slot uses** | `ksx slot list [--profile TITLE]` (read-only, daemon-free); `ksx slot assign --slot N --preset NAME [--profile TITLE] [--reload] [--json]`; pipe `slot-assign`; writer `ksx-backend/src/slots.rs` | **The cabinet's "Slots" screen** — pick a slot, pick a preset, confirm the pad bounce. `ControlSource::assign_slot` → the same verb. **Studio no longer calls `assign_slot` at all (2026-08-25).** Its historical face was `POST /setup/slot`, step 2 of the setup checklist, with the pad bounce stated above the button rather than flashed after it; the cutover deleted the route with the page. A slot's preset is now a property of the staged controller on `/redesign` and reaches disk with the rest of the draft at Save, so the capability is intact and the shared *writer* seam with the CLI is not | exists — CLI + pipe + cabinet; Studio writes it through the staged save instead |
...
| WinUSB claim / release / status | Advanced CLI: `ksx winusb status` is read-only; legacy operator `claim`/`release` remain explicit. Supported customer mutation requires an installed build | **Studio `/redesign` is primary and installed-only**: `POST /redesign/capture/prepare` and `POST /redesign/capture/release` carry the exact selected selector+instance, three prepare confirmations (tested spare, rebind consequence, machine-local certificate), one release confirmation, UAC through the fixed GUI helper, fresh exact re-survey, then guarded backend transition. Portable copies refuse preparation/release; the linked `/devices` tool remains the read-only hardware and recovery inventory | exists — CLI status + typed MachineSource + Studio |
| Autostart | `ksx autostart --enable/--disable/--status` (validates the config before registering); hidden installed `uninstall-quiesce` removes only fixed `ksx\autostart` and freshly proves it absent | The backend/task contract remains; its redesigned Settings/Library control is deferred and there is no Studio form after the hard cutover | exists — CLI + backend; no Studio UI |
| Install controller output drivers | `ksx install-drivers [--dry-run] [--yes]` for ViGEmBus; the checked HIDMaestro installer task owns its pinned package/bootstrap — neither ordinary runtime self-elevates to install | Installer checkboxes are primary; Studio `/redesign` reports only the backends required by the currently staged supported personas through `MachineSource::controller_outputs`, while the linked `/pads` tool retains detailed ViGEm bus inspection. HIDMaestro's package evidence stays `verified-on-play`; nothing on it pretends its protected endpoint exists before Play or can run either general installer. WinUSB's fixed exact-device helper is a separate capture capability/consent boundary, not this row | exists |
| Export config as JSON | `ksx config export [--what config\|games\|presets\|all] [--preset NAME] [--out PATH\|-] [--compact] [--json]` — the document on stdout (so it pipes), the summary on stderr | `MachineSource::config_export` still returns the document in memory (no path, no stdout), preserving the future surface contract. The retired `/nocturne/export.json` download does not survive the hard cutover; Settings/Library is deferred | exists — CLI + typed API; no Studio UI |
| Import config from JSON | `ksx config import <PATH\|-> [--what …] [--dry-run] [--yes] [--force] [--json]` — validated first, DRY RUN unless `--yes`, timestamped `.bak` of every overwritten file | `MachineSource::config_import` still takes the document, not a path, and keeps dry-run/apply/force and backup semantics. The retired `/nocturne/import` form and its 8 MB route limit remain design evidence only; Settings/Library is deferred | exists — CLI + typed API; no Studio UI |
...
only senders were the tray thread and the headless stdin reader. The pipe is now
the typed `ControlSource` seam shared by shell and Studio. Session action verbs
still enqueue the exact `DaemonCommand` values the tray menu produces and read
the same `DaemonState` snapshot. Editor and bounded-observer verbs delegate to
their daemon-owned services: mapping writers, Learn, and the simultaneous-input
tester. The pipe never calls a time-critical pipeline loop directly, and it
runs on plain threads — no async runtime, so E7 rule A (the default build links
...
→ {"verb":"input-test-start","selector":"usb:d209:0430:00","duration_ms":30000}
← {"ok":true,"state":"listening","generation":7,
   "selector":"usb:d209:0430:00","remaining_ms":29996,
   "held":[],"seen":[],"peak":0,"events":0,"dropped":0,
   "rollover_visibility":"unavailable","detail":"KSX currently observes …"}
→ {"verb":"input-test-poll"}
← {"ok":true,"state":"timeout","generation":7,"remaining_ms":null,
   "held":[],"seen":["J","K","L"],"peak":3,"events":6,"dropped":0,
...
**simultaneous-input test semantics**: this is a different observation over the
same exclusive physical-listener boundary. `input-test-start` takes one exact
canonical selector and a total wall-clock budget of 1–60 seconds; preparation,
fresh device resolution and listening all spend that one budget. It is refused
while Play is running/starting or Learn is listening/releasing. A Play
transition during preparation or listening stops the test before another
signal is accepted. Ordinary keyboards are observed through Raw Input; a
WinUSB-claimed panel is observed through the daemon's existing panel tap. The
fresh resolution must still name that exact selected source — the service does
not fall back to another keyboard when the board disappears or cannot type.
The observer holds the same machine-wide lease as standalone Play and
persistent encoder maintenance for its complete lifetime, so a separate
process cannot start Play or read/write the board's chart underneath the test;
whichever operation fails to acquire that lease refuses without opening its
hardware path.

The result measures **host signals KSX decoded**, not physical switches and not
the encoder's advertised rollover. `held` is the current set, `seen` is the
distinct set across the attempt, `peak` is the largest held set, and `events`
counts accepted make/break transitions. Two physical terminals assigned the
same key collapse into one signal before KSX can distinguish them. Raw Input
does not expose the original USB ErrorRollOver report, so
`rollover_visibility:"unavailable"` is explicit; a high peak is evidence of
what this attempt delivered, never proof of NKRO.

Start returns immediately with a monotonically increasing `generation`; poll
returns a bounded snapshot (`idle`, `listening`, `timeout`, `cancelled`, or
`failed`). Cancellation is generation-stamped, so a stale Studio tab or shell
cannot stop a newer listener. The wire still accepts an omitted generation for
old clients, but new Studio and `ksx input-test cancel` both require the exact
number. A terminal `state:"failed"` is a readable diagnostic snapshot and may
therefore carry `ok:true`; it still means the observer failed. The CLI prints
...
pipe** looks identical to "not running" on this surface. Historical note: the
legacy `/nocturne` system reading included a process-list row that could catch
that case; it used to sit on the still older status page, which is why earlier
notes called it only "the Studio page".

**Clients**: `ksx session status|start [--game TITLE]|stop|reload|quit` and
`ksx input-test start|poll|cancel` (`--json` prints the raw response; exit 0 =
done, 1 = daemon refusal / observer failure / pipe error, 2 = no control
channel; initially absent is exit 0 for session `quit` alone), plus Studio's
session panel. The simultaneous-input dialog is deferred even though its API
contract remains. The E5 MCP shim gets the
same channel for free.

## The honest gaps

1. ~~No non-interactive mapping verbs yet.~~ **CLOSED (M7 slice, 2026-08-05)**:
   `ksx map` + pipe `map`/`learn-*` + Studio's mapper (historically `/map`, then
   `/nocturne`, and now `/redesign`), all over one writer. ~~Still open from the
   E5 family: `ksx slot assign`.~~ **CLOSED
...
2. ~~**Per-slot persona is a TOML edit today.**~~ **CLOSED**, and then partly
   re-opened in a way worth spelling out. `ksx slot assign --persona` and the
   typed pipe/control request still share one writer, and `/redesign` now carries
   the canonical persona roster in its reversible stage (after the intermediate
   `/nocturne` workbench). What is no longer true is the middle clause: Studio's
   historical `/setup` form posted `slot-assign` directly and therefore shared
   that writer literally, and the 2026-08-25 cutover removed it. Studio reaches
   the same config through the staged save instead,
   which is a different seam with the same destination — one to watch, because
   "the CLI and the browser call the same function" is a stronger guarantee than
   "they end up writing the same field". The hand-editable TOML remains a
   first-class bulk interface and still round-trips.
