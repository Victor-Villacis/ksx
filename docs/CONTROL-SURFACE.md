...
|---|---|---|---|
| Start emulation | `ksx run` (foreground session); daemon: tray "Start emulation" / headless stdin `start` / pipe `start` / `ksx session start [--game TITLE]` — all → `DaemonCommand::Start` | M9: enqueue `DaemonCommand::Start` on the control loop the UI hosts in-process. **M10: Studio's Start button POSTs `/nocturne/play` → pipe `start` → the same command (live)** | exists — pipe + CLI + Studio live |
| Stop emulation | tray "Stop emulation" / stdin `stop` / pipe `stop` / `ksx session stop` → `DaemonCommand::Stop`; Ctrl+Alt+Del is the capture-thread emergency stop. LeftCtrl ×5 only toggles keyboard capture off/on and deliberately leaves the session running | M9: `DaemonCommand::Stop`. **M10: Studio's Stop button POSTs `/nocturne/stop` → pipe `stop` (live).** Escapes are deliberately NOT a GUI concern — see invariants | exists — pipe + CLI + Studio live |
...
| **Which preset a slot uses** | `ksx slot list [--profile TITLE]` (read-only, daemon-free); `ksx slot assign --slot N --preset NAME [--profile TITLE] [--reload] [--json]`; pipe `slot-assign`; writer `ksx-backend/src/slots.rs` | **The cabinet's "Slots" screen** — pick a slot, pick a preset, confirm the pad bounce. `ControlSource::assign_slot` → the same verb. **Studio no longer calls `assign_slot` at all (2026-08-25).** Its face was `POST /setup/slot`, step 2 of the setup checklist, with the pad bounce stated above the button rather than flashed after it; the cutover deleted the route with the page. A slot's preset is now a property of the staged controller on `/nocturne` and reaches disk with the rest of the draft at Save, so the capability is intact and the shared *writer* seam with the CLI is not | exists — CLI + pipe + cabinet; Studio writes it through the staged save instead |
...
| WinUSB claim / release / status | Advanced CLI: `ksx winusb status` is read-only; legacy operator `claim`/`release` remain explicit. Supported customer mutation requires an installed build | **Studio `/nocturne` is primary and installed-only**: exact selected selector+instance, three prepare confirmations (tested spare, rebind consequence, machine-local certificate), one release confirmation, UAC through the fixed GUI helper, fresh exact re-survey, then guarded backend transition. Portable copies refuse preparation/release | exists — CLI status + typed MachineSource + Studio |
...
| Install controller output drivers | `ksx install-drivers [--dry-run] [--yes]` for ViGEmBus; the checked HIDMaestro installer task owns its pinned package/bootstrap — neither ordinary runtime self-elevates to install | Installer checkboxes are primary; Studio `/nocturne` reports only the backends required by the currently staged supported personas through `MachineSource::controller_outputs`, and the same page's system reading inventories both installed stacks. HIDMaestro's package evidence stays `verified-on-play`; nothing on it pretends its protected endpoint exists before Play or can run either general installer. WinUSB's fixed exact-device helper is a separate capture capability/consent boundary, not this row | exists |
| Export config as JSON | `ksx config export [--what config\|games\|presets\|all] [--preset NAME] [--out PATH\|-] [--compact] [--json]` — the document on stdout (so it pipes), the summary on stderr | M9: same verb in-process; the document IS the payload a form would populate. M10: **live** — `MachineSource::config_export` returns the document IN MEMORY (no path, no stdout), and Studio's Configuration menu serves it as a download from `GET /nocturne/export.json`, under the label "Export the configuration (download)" | exists — CLI + api + Studio |
| Import config from JSON | `ksx config import <PATH\|-> [--what …] [--dry-run] [--yes] [--force] [--json]` — validated first, DRY RUN unless `--yes`, timestamped `.bak` of every overwritten file | M9: same verb, preserving dry-run-first. M10: **live and local only** — `MachineSource::config_import` takes the DOCUMENT, not a path, and keeps the CLI's consent shape exactly (`apply` is what writes; without it the answer is a report). Studio's Configuration menu posts it at `POST /nocturne/import`, behind an "Import a configuration…" disclosure so the consent box is read before the textarea is filled. ⚠ The old route carried an explicit 8 MB `DefaultBodyLimit` and the new one does not, while the refusal copy still names 8 MB — so a whole-cabinet document between axum's 2 MB default and that promised ceiling fails with a bare 413 under a sentence quoting the wrong number. Still loopback-only besides: a REMOTE surface that can replace the whole config wants the pairing token first | exists — CLI + api + Studio |
...
row on `/nocturne` is what catches that case. (Worth naming the page now that
there is only one: the reading used to sit on the status page, and "the Studio
page" was ambiguous even then.)
...
1. ~~No non-interactive mapping verbs yet.~~ **CLOSED (M7 slice, 2026-08-05)**:
   `ksx map` + pipe `map`/`learn-*` + Studio's mapper (`/map` then, `/nocturne`
   now), all over one writer. ~~Still open from the E5 family: `ksx slot assign`.~~ **CLOSED
...
2. ~~**Per-slot persona is a TOML edit today.**~~ **CLOSED**, and then partly
   ...
   typed pipe/control request still share one writer, and `/nocturne` carries
   the canonical persona roster in its reversible stage. What is no longer true
   is the middle clause: Studio's `/setup` form posted `slot-assign` directly
   and therefore shared that writer literally, and the 2026-08-25 cutover
   removed it. Studio reaches the same config through the staged save instead,
