...
| Export config as JSON | `ksx config export [--what config\|games\|presets\|all] [--preset NAME] [--out PATH\|-] [--compact] [--json]` — the document on stdout (so it pipes), the summary on stderr | M9: same verb in-process; the document IS the payload a form would populate. M10: **live** — `MachineSource::config_export` returns the document IN MEMORY (no path, no stdout), and Studio's explicitly deferred Settings/Library menu serves it as a download from `GET /nocturne/export.json`, under the label "Export the configuration (download)" | exists — CLI + api + Studio |
| Import config from JSON | `ksx config import <PATH\|-> [--what …] [--dry-run] [--yes] [--force] [--json]` — validated first, DRY RUN unless `--yes`, timestamped `.bak` of every overwritten file | M9: same verb, preserving dry-run-first. M10: **live and local only** — `MachineSource::config_import` takes the DOCUMENT, not a path, and keeps the CLI's consent shape exactly (`apply` is what writes; without it the answer is a report). Studio's explicitly deferred Settings/Library menu posts it at `POST /nocturne/import`, behind an "Import a configuration…" disclosure so the consent box is read before the textarea is filled. The route restores the explicit 8 MB `DefaultBodyLimit` promised by its refusal copy, so a whole-cabinet document is not silently cut down to axum's 2 MB default. Still loopback-only besides: a REMOTE surface that can replace the whole config wants the pairing token first | exists — CLI + api + Studio |
...
session panel and simultaneous-input dialog (below). The E5 MCP shim gets the
