...
on the first POLL is invisible to it. `poll()` fetched `/api/map` (then
`/api/nocturne`; the current core equivalent is `/api/redesign`, and the blind
spot is the same shape) with no `slot=`, and
