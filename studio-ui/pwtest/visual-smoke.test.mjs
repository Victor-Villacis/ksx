...
const BENCH_A_SELECTOR = "usb:046d:c545:00";
const BENCH_B_SELECTOR = "usb:d209:0430:00";
const BENCH_A = deviceInstanceId(BENCH_A_SELECTOR); // Logitech G915 TKL
...
const seedRealDeviceWorkbench = (page) => page.addInitScript((selectors) => {
  localStorage.setItem(
    "ksx-redesign-canvas",
    JSON.stringify({ widgets: {}, bench: selectors }),
  );
...
describe("redesign canvas interaction chrome", () => {
  test("Inspector, minimap, position fields, and shortcut scope stay coherent", async () => {
    ...
    try {
      ...
      const minimapRepresentedWidth = async () => page.evaluate(({ benchA, benchB }) => {
        const area = document.querySelector(".forma-canvas-navigator-items");
        const camera = document.querySelector(".forma-canvas-navigator-viewport");
        const markerA = area?.querySelector(`[data-instance-id="${benchA}"]`);
        const markerB = area?.querySelector(`[data-instance-id="${benchB}"]`);
        const itemA = document.querySelector(
          `.forma-canvas-stage > [data-instance-id="${benchA}"]`,
        );
      ...
      const mappedWorldPoint = await page.evaluate((benchA) => {
        const area = document.querySelector(".forma-canvas-navigator-items");
        const marker = area?.querySelector(`[data-instance-id="${benchA}"]`);
