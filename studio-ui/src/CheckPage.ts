import { CheckIsland } from "./CheckIsland";

// The SSR root — a compile-time artifact with one job and zero runtime
// presence, exactly like PadsPage.ts. Returning `CheckIsland()` makes the whole
...
// island attributes the client runtime activates against.
// `parseEntryPoint` picks the imported `*Page` that is NOT in check.ts's
