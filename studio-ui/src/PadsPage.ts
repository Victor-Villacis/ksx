import { PadsIsland } from "./PadsIsland";

// The SSR root — a compile-time artifact with one job and zero runtime
// presence, exactly like StatusPage.ts. Returning `PadsIsland()` makes the
// whole screen one island: the compiler inlines its h() tree between
// ISLAND_START/ISLAND_END, so the Rust walker SSRs the full page and stamps
// the island attributes the client runtime activates against.
// `parseEntryPoint` picks the imported `*Page` that is NOT in pads.ts's
