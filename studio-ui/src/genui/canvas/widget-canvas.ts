...
export class WidgetCanvas {
  ...
  removeItem(item: HTMLElement, options: { selectFallback?: boolean } = {}): void {
    ...
    item.remove();
    const removedWasSelected = id ? this.#selectedIds.delete(id) : false;
    if (id === this.#activeId) {
      this.#activeId = null;
      const remaining = Array.from(this.#items.values());
      const selectedRemaining = Array.from(this.#selectedIds, (selectedId) =>
        this.#items.get(selectedId)
      ).filter((candidate): candidate is HTMLElement => Boolean(candidate));
      const fallback = selectedRemaining.at(-1) ??
        (options.selectFallback !== false ? remaining.at(-1) ?? null : null);
      if (fallback) {
        if (selectedRemaining.length === 0) {
          this.setActive(fallback);
        } else {
          // Removing the primary from a multi-selection promotes the latest
          // surviving member without collapsing the rest of that selection.
          this.#activeId = this.#itemId(fallback);
          this.#applySelectionClasses();
          this.#syncNavigationFocusTargets();
          this.#onActiveChange(fallback);
      ...
      // A non-primary member disappearing still changes the host-visible
      // selection (for example, the redesign Inspector's multi-select body).
      this.#applySelectionClasses();
      this.#syncNavigationFocusTargets();
      this.#onSelectionChange(this.selectedItems());
    }
