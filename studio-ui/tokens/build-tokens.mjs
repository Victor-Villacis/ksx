...
export function buildTokens({ authoredCss }) {
  ...
  for (const t of themes) {
    ...
    if (/["\\]/.test(t.label)) {
      throw new Error(`theme '${t.id}' label must not contain quotes or backslashes`);
    }
  ...
  let css =
    ...
    "   Regenerate: tools/studio-env/build-assets.ps1 — CI pins this file to a\n" +
  ...
  for (const b of semantic.blocks) {
    ...
    // pages so an explicit data-theme choice wins in both OS schemes.
    // Exactly ONE theme is the system counterpart — id "light" — so a
    // second light-scheme theme someday is stamp-only, not system-follow.
    if (b.name === "theme-base") {
      for (const t of themes.filter((x) => x.id === "light")) {
        css += "\n";
  ...
  // ── the stamped theme blocks (TK2) ───────────────────────────────────────
  // One :root[data-theme="id"] block per theme, after the base tiers. An
  // explicit choice wins in BOTH OS schemes because the system-follow light
  // block above is guarded to :not([data-theme]). "dark" needs no block: the
  // base :root IS the dark theme, and any stamp defeats the light guard.
  // Content comes from the same source as the blocks above — the contrast
  // gate cross-checks any id that mirrors an existing map and runs every
  // floor over any id that does not.
  css +=
    "\n/* ── Stamped themes: an explicit data-theme choice on <html>. Same\n" +
    "   source as the blocks above; see studio-ui/tokens/themes/. ── */\n";
  for (const t of themes) {
    css += `:root[data-theme="${t.id}"] {\n`;
    for (const e of t.tokens) if (e.cssName) css += `  ${e.cssName}: ${e.$value};\n`;
    css += "}\n";
  }
  ...
  // or line-continuation inside a rule would defeat the match. color-scheme
  // rides along at every step (TK2): without it, scrollbars and form
  // controls first-paint in the OS scheme while the sheet is pending — the
  // same wrong-theme flash this CSS exists to kill, at smaller scale. The
  // per-theme rules need no ordering tricks: html[data-theme=x] (0,1,0) and
  // html[data-theme=x] body (0,1,2) outweigh the media-scoped html/body
  // rules by specificity in both directions.
  let personality =
    "html{color-scheme:dark}" +
    `body{background:${hex("dark", "--bg")};color:${hex("dark", "--text")};margin:0}` +
    "@media (prefers-color-scheme:light){html{color-scheme:light}" +
    `body{background:${hex("light", "--bg")};color:${hex("light", "--text")}}}`;
  for (const t of roster) {
    personality +=
      `html[data-theme=${t.id}]{color-scheme:${t.scheme}}` +
      `html[data-theme=${t.id}] body{background:${hex(t.id, "--bg")};color:${hex(t.id, "--text")}}`;
  }
  ...
  const rustModule =
    "// Generated by studio-ui/tokens/build-tokens.mjs — DO NOT EDIT.\n" +
    "// Source of truth: studio-ui/tokens/; regenerate: tools/studio-env/build-assets.ps1\n" +
  ...
  // ── the theme roster file, for the browser suite ─────────────────────────
