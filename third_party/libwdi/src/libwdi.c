...
#include "logging.h"
#include "tokenizer.h"
#include "embedded.h"	// deterministic KSX prepare-provider resource slice
...
static const char* driver_name[WDI_NB_DRIVERS-1] = {"", "", "", ""};
static const char* inf_template[WDI_NB_DRIVERS-1] = {"winusb.inf.in", NULL, NULL, NULL};
static const char* cat_template[WDI_NB_DRIVERS-1] = {"winusb.cat.in", NULL, NULL, NULL};
static const char* ms_compat_id[WDI_NB_DRIVERS-1] = {"MS_COMP_WINUSB", NULL, NULL, NULL};
...
/* GetVersionEx/VerifyVersionInfo are application-manifest sensitive on
 * Windows 10 and 11. The provider cannot depend on its caller's manifest, so
 * use ntdll's manifest-independent version query for the production gate. */
static BOOL ksx_is_windows_10_or_later(void)
{
	typedef LONG (WINAPI *RtlGetVersion_t)(POSVERSIONINFOW);
	HMODULE ntdll;
	RtlGetVersion_t rtl_get_version;
	OSVERSIONINFOW version_info = { 0 };
	LONG status;

	ntdll = GetModuleHandleW(L"ntdll.dll");
	if (ntdll == NULL)
		return FALSE;
	rtl_get_version = (RtlGetVersion_t)(uintptr_t)GetProcAddress(ntdll, "RtlGetVersion");
	if (rtl_get_version == NULL)
		return FALSE;
	version_info.dwOSVersionInfoSize = sizeof(version_info);
	status = rtl_get_version(&version_info);
	return (status >= 0) && (version_info.dwMajorVersion >= 10);
}
...
// The KSX DLL is intentionally an in-box WinUSB prepare provider, not a general
// libwdi distribution. Keep this answer as narrow as the exported ABI.
...
#define KSX_DEVICE_DESCRIPTION "KSX WinUSB Keyboard Interface"
#define KSX_VENDOR_NAME "KSX"
#define KSX_DEVICE_GUID "{B8B2D1F8-6E0E-4C7F-9E5A-3A9C1D6F2E10}"
#define KSX_INF_PREFIX "ksx-winusb-"
#define KSX_CERT_SUBJECT_PREFIX "CN=KSX WinUSB "

static BOOL ksx_is_safe_inf_name(const char* name)
{
	size_t i, length, prefix_length = strlen(KSX_INF_PREFIX);

	if (name == NULL)
		return FALSE;
	length = strlen(name);
	if ((length <= prefix_length + 4) || (length >= 120) ||
		(strncmp(name, KSX_INF_PREFIX, prefix_length) != 0) ||
		(strcmp(name + length - 4, ".inf") != 0))
		return FALSE;
	for (i = prefix_length; i < length - 4; i++) {
...
static BOOL ksx_is_safe_cert_subject(const char* subject)
{
	size_t i, length, prefix_length = strlen(KSX_CERT_SUBJECT_PREFIX);
	size_t hex_digits = 0;

	if (subject == NULL)
		return FALSE;
	length = strlen(subject);
	if ((length < prefix_length + 32) || (length >= 96) ||
		(strncmp(subject, KSX_CERT_SUBJECT_PREFIX, prefix_length) != 0))
		return FALSE;
	for (i = prefix_length; i < length; i++) {
		if (isxdigit((unsigned char)subject[i]))
			hex_digits++;
		else if (subject[i] != '-')
			return FALSE;
	}
...
/* Rust creates and ACL-verifies the transaction directory. The provider
 * independently requires a canonical, existing, local, non-reparse directory
 * and never creates it or changes its owner/DACL. */
static BOOL ksx_is_existing_output_directory(const char* path)
{
	LPWSTR wide_path = NULL;
	WCHAR full_path[MAX_PATH];
	DWORD full_length, attributes;
	...
	if (path == NULL)
		return FALSE;
	length = strlen(path);
	if ((length <= 3) || (length >= MAX_PATH) || !isalpha((unsigned char)path[0]) ||
		(path[1] != ':') || ((path[2] != '\\') && (path[2] != '/')) ||
		(path[length - 1] == '\\') || (path[length - 1] == '/'))
		return FALSE;
	wide_path = utf8_to_wchar(path);
	if (wide_path == NULL)
		goto out;
	full_length = GetFullPathNameW(wide_path, ARRAYSIZE(full_path), full_path, NULL);
	if ((full_length == 0) || (full_length >= ARRAYSIZE(full_path)) ||
		(_wcsicmp(wide_path, full_path) != 0))
		goto out;
	...
	if ((attributes == INVALID_FILE_ATTRIBUTES) ||
		...
		goto out;
	directory = CreateFileW(full_path, FILE_READ_ATTRIBUTES,
...
// Tokenize only the provider-owned canonical external WinUSB INF template.
static long wdi_tokenize_ksx_external_inf(const char* src, char** dst,
							  ...
							  const char* tok_prefix, const char* tok_suffix, int recursive)
{
	...
	DWORD bytes_read = 0;
	long size = 0, ret = -ERROR_RESOURCE_DATA_NOT_FOUND;
	char* buffer = NULL;
	long read_offset, write_offset;

	file = CreateFileU(src, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
		FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OPEN_REPARSE_POINT, NULL);
	if (file == INVALID_HANDLE_VALUE)
		goto out;
	if (!GetFileInformationByHandle(file, &file_info) ||
		(file_info.dwFileAttributes & (FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_REPARSE_POINT)) ||
		!GetFileSizeEx(file, &file_size) || (file_size.QuadPart <= 0) ||
		(file_size.QuadPart > (LONGLONG)(2 * sizeof(ksx_winusb_inf_template)))) {
		wdi_err("External INF template has an invalid size");
		ret = -ERROR_INVALID_DATA;
		goto out;
	}
	size = (long)file_size.QuadPart;
	...
	/* Normalize CRLF to LF, rejecting lone CR. The resulting bytes must match
	 * the provider-owned in-box WinUSB template exactly. This prevents an
	 * elevated caller from repurposing the signing provider for another
	 * service, coinstaller, architecture, or arbitrary catalog member. */
	for (read_offset = 0, write_offset = 0; read_offset < size; read_offset++) {
		if (buffer[read_offset] == '\r') {
			if ((read_offset + 1 >= size) || (buffer[read_offset + 1] != '\n')) {
				wdi_err("External INF template contains a lone carriage return");
				ret = -ERROR_INVALID_DATA;
				goto out;
			}
			continue;
	...
	if ((write_offset != (long)(sizeof(ksx_winusb_inf_template) - 1)) ||
		(memcmp(buffer, ksx_winusb_inf_template, (size_t)write_offset) != 0)) {
		wdi_err("External INF template is not the canonical KSX in-box WinUSB template");
		ret = -ERROR_INVALID_DATA;
		goto out;
	}
	ret = tokenize_string(buffer, write_offset, dst, token_entities,
		tok_prefix, tok_suffix, recursive);
...
// Prepare a signed catalog for a caller-rendered, in-box WinUSB INF.
								  ...
								  const char* inf, struct wdi_options_prepare_driver* options)
{
	...
	const char* cat_list[1];
	char drv_path[MAX_PATH] = { 0 }, inf_path[MAX_PATH] = { 0 };
	char cat_path[MAX_PATH] = { 0 }, hw_id[40];
	char *cat_name = NULL, *dst = NULL, *inf_name;
	wchar_t *wdst = NULL;
	int r = WDI_ERROR_OTHER;
	long inf_file_size;
	size_t body_chars, wrote_bom, wrote_body;
	int close_result;
	BOOL inf_output_started = FALSE, cat_output_started = FALSE;
	FILE* fd = NULL;

	MUTEX_START;

	if (!ksx_is_windows_10_or_later()) {
		wdi_err("The KSX prepare provider requires Windows 10 or newer");
		r = WDI_ERROR_NOT_SUPPORTED;
		goto out;
	}

	if ((device_info == NULL) || (path == NULL) || (inf == NULL) || (options == NULL)) {
		wdi_err("A required prepare-provider parameter is NULL");
		r = WDI_ERROR_INVALID_PARAM;
		goto out;
	}
	if ((options->driver_type != WDI_WINUSB) || (!options->external_inf) ||
		options->disable_cat || options->disable_signing || options->use_wcid_driver ||
		(options->vendor_name == NULL) || (strcmp(options->vendor_name, KSX_VENDOR_NAME) != 0) ||
		(options->device_guid == NULL) || (_stricmp(options->device_guid, KSX_DEVICE_GUID) != 0) ||
		!ksx_is_safe_cert_subject(options->cert_subject) ||
		(device_info->next != NULL) || (device_info->vid == 0) || (device_info->pid == 0) ||
		((device_info->is_composite != FALSE) && (device_info->is_composite != TRUE)) ||
		(!device_info->is_composite && (device_info->mi != 0)) ||
		(device_info->desc == NULL) || (strcmp(device_info->desc, KSX_DEVICE_DESCRIPTION) != 0) ||
		(device_info->driver != NULL) || (device_info->device_id != NULL) ||
		(device_info->hardware_id != NULL) || (device_info->compatible_id != NULL) ||
		(device_info->upper_filter != NULL) || (device_info->driver_version != 0)) {
		wdi_err("KSX accepts only signed external-INF WinUSB preparation with an explicit GUID and unique certificate subject");
		r = WDI_ERROR_INVALID_PARAM;
		goto out;
	}
	if (!IsUserAnAdmin()) {
		wdi_err("Signed catalog preparation requires an elevated KSX helper");
	...
	if (!ksx_is_safe_inf_name(inf_name)) {
		wdi_err("INF filename is not a canonical KSX WinUSB package name");
		r = WDI_ERROR_INVALID_PARAM;
		goto out;
	}

	if (!ksx_is_existing_output_directory(path)) {
		wdi_err("Output path must be a canonical existing protected local directory");
		r = WDI_ERROR_ACCESS;
		goto out;
	}
	...
	if (_stricmp(inf, inf_path) != 0) {
		wdi_err("External INF template must be the exact output-directory member");
		r = WDI_ERROR_INVALID_PARAM;
		goto out;
	}
	...
	static_strcpy(inf_entities[DEVICE_DESCRIPTION].replace, KSX_DEVICE_DESCRIPTION);
	...
	static_strcpy(inf_entities[DEVICE_INTERFACE_GUID].replace, KSX_DEVICE_GUID);
	static_strcpy(inf_entities[DEVICE_MANUFACTURER].replace, KSX_VENDOR_NAME);
	/* Fixed source-controlled DriverVer values make the rendered INF
	 * deterministic and independently verifiable by the Rust caller. */
	static_strcpy(inf_entities[DRIVER_DATE].replace, "01/01/2026");
	static_strcpy(inf_entities[DRIVER_VERSION].replace, "1.5.1.788");

	// Tokenize the already exact-compared canonical external INF.
	inf_file_size = wdi_tokenize_ksx_external_inf(inf, &dst,
		inf_entities, "#", "#", 0);
	if (inf_file_size > 0) {
		fd = fopenU(inf_path, "wb");
		if (fd == NULL) {
			wdi_err("Failed to create file: %s", inf_path);
			r = WDI_ERROR_ACCESS;
			goto out;
		}
		inf_output_started = TRUE;
		...
		body_chars = wcslen(wdst);
		wrote_bom = fwrite(&bom, sizeof(bom), 1, fd);
		wrote_body = fwrite(wdst, sizeof(wchar_t), body_chars, fd);
		close_result = fclose(fd);
		fd = NULL;
		if ((wrote_bom != 1) || (wrote_body != body_chars) || (close_result != 0)) {
			wdi_err("Could not completely write and close '%s'", inf_path);
			r = WDI_ERROR_IO;
			goto out;
		}
		fd = NULL;
	...
	static_sprintf(hw_id, "USB\\%s", inf_entities[DEVICE_HARDWARE_ID].replace);

	// KSX never accepts test-signing mode as a fallback. Catalog construction
	// and signing are mandatory regardless of machine policy.
	cat_output_started = TRUE;
	if (!CreateCat(cat_path, hw_id, drv_path, cat_list, 1)) {
		wdi_err("Could not create cat file");
		r = WDI_ERROR_CAT_MISSING;
		goto out;
	}
	if (!SelfSignFile(cat_path, options->cert_subject)) {
		wdi_err("Could not safely sign cat file");
...
out:
	...
	if (r != WDI_SUCCESS) {
		if (cat_output_started && (cat_path[0] != 0) &&
			(GetFileAttributesU(cat_path) != INVALID_FILE_ATTRIBUTES) && !DeleteFileU(cat_path))
			wdi_warn("Could not remove partial catalog '%s'", cat_path);
		if (inf_output_started && (inf_path[0] != 0) &&
			(GetFileAttributesU(inf_path) != INVALID_FILE_ATTRIBUTES) && !DeleteFileU(inf_path))
			wdi_warn("Could not remove partial INF '%s'", inf_path);
	}
	safe_free(dst);
	safe_free(wdst);
	safe_free(cat_name);
