...
/*
 * KSX deliberately controls the DLL surface through libwdi.def. Upstream
 * marks every public declaration dllexport, which would make enumeration,
 * installer, and certificate-import entry points reachable even when the
 * module definition file omitted them. Keeping this macro empty leaves only
 * the three prepare-provider exports named in libwdi.def.
 */
#define LIBWDI_EXP
